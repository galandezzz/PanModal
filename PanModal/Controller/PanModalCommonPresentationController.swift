...
public protocol PanModalTransitionCoordinator {
    func animate(alongsideTransition animations: @escaping () -> Void, completion: ((Bool) -> Void)?)
}

extension PanModalTransitionCoordinator {
   public func animate(alongsideTransition animations: @escaping () -> Void) {
        animate(alongsideTransition: animations, completion: nil)
    }
}

protocol PanModalCommonPresentationControllerDelegate: AnyObject {
...
final class PanModalCommonPresentationController: NSObject {
    /**
     Constants
     */
    struct Constants {
    ...
    weak var delegate: PanModalCommonPresentationControllerDelegate?
    ...
    private lazy var backgroundView: DimmedView = {
        ...
        if presentable?.passesTouchesThroughDimmedView ?? false {
            if containerView?.isDescendant(of: presentingViewController.view) == false {
        ...
        view.didTap = { [weak self] _ in
            ...
            if self.presentable?.allowsTapToDismiss == true {
                self.delegate?.controllerNeedDismiss(self)
            }
        }
        return view
    }()
    ...
    var containerView: UIView? {
        return delegate?.containerView(for: self)
    }
    ...
    func presentationTransitionWillBegin() {
        ...
        guard let coordinator = delegate?.transitionCoordinator(for: self) else {
            backgroundView.dimState = .max
            return
        }
        
        coordinator.animate { [weak self] in
...
extension PanModalCommonPresentationController {
...
private extension PanModalCommonPresentationController {
        ...
        default:
            ...
            if isVelocityWithinSensitivityRange(velocity.y) {
                ...
                if velocity.y < 0 {
                    ...
                    delegate?.controllerNeedDismiss(self)
                }
...
extension PanModalCommonPresentationController: UIGestureRecognizerDelegate {
